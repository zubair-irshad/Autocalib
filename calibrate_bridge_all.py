...
# python calibrate_bridge_all.py --verbose
...
parser.add_argument('--calib_script_path', type=str, default="/data/user_data/wenhsuac/chenyuzhang/backup/drrobot/calibrate_bridge_single_alltracker.py")
...
if os.path.exists(args.traj_paths):
    ...
    wildcards = ['*'] * 5
    pattern = os.path.join(args.raw_path, *wildcards, 'raw', 'traj_group*', 'traj*')
...
for traj_path in tqdm(traj_paths[:], desc="Processing trajectories"):
    ...
    calib_cmd = [
        'conda', 'run', '-n', 'dr_test', 'python', args.calib_script_path,
        '--model_path', 'output/widow0',
        '--scene_path', traj_path,
    ...
    try:
        if args.verbose:
            # print(f"Running calibration for {traj_path} with command: {' '.join(calib_cmd)}")
            # print('Current working directory:', calib_project_dir)
            subprocess.run(calib_cmd, check=True, cwd=calib_project_dir)
            if os.path.exists(args.dict_path):
                with open(args.dict_path, 'rb') as f:
                    calib_dict = pickle.load(f)
                    print(f"Updated dictionary {calib_dict}")
        else:
            subprocess.run(calib_cmd, check=True, cwd=calib_project_dir, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    except subprocess.CalledProcessError as e:
