...
def optimize(optimize_camera, optimize_joints, camera_lr, joints_lr, optimization_steps, fwd_flows, fwd_valids, frame_idx):
    ...
    for step in tqdm(range(optimization_steps)):
        ...
        for camera, joint_pose in zip(all_cameras, all_joint_poses):
            ...
            distance_squared = torch.sum((new_image - 1)**2, dim=0)  # Shape [H, W]
            sigma = 0.1  # Example value; adjust as needed
            mask_tensor = 1 - torch.exp(-distance_squared / sigma**2)
        ...
        if use_direct_feat:
            ...
            visualize_features(features_final, f'inspection_features1/features_{0}.png')
        ...
        if tracking_loss:
            ...
            motion_mask = (torch.norm(fwd_flows, dim=1, keepdim=True) > 4)
            ...
            flow_loss = flow_loss * 0 + flow_mask_loss 
...
def compute_loss(optimize_camera, optimize_joints, camera_lr, joints_lr, optimization_steps, fwd_flows, fwd_valids):
    ...
    for step in tqdm(range(optimization_steps)):
        ...
        if tracking_loss and step == 0:
            ...
            motion_idx = motion_mask.sum(dim=(1, 2, 3)) > 1000
            motion_idx = motion_idx.float().unsqueeze(1).unsqueeze(1).unsqueeze(1)
            motion_mask = motion_mask.float() # [T-1, 1, H, W]

        if tracking_loss and save_inspection:
            for i in range(1, 2):
                print(gaussian_flows.shape, 'gaussian flows shape')
                flow_2d_np_render = gaussian_flows[i-1].squeeze(0).permute(1, 2, 0).detach().cpu().numpy()  # [H, W, 2
                flow_color_mujoco = flow_to_color(flow_2d_np_render) # np array
                flow_visual = Image.fromarray((flow_color_mujoco * 255).astype(np.uint8))
                print(f'saving to inspection_features1/render_flow_{i}.png')
                flow_visual.save(f'inspection_features1/render_flow_{i}.png')
                
                flow_2d_mujoco = fwd_flows[i-1].permute(1, 2, 0).detach().cpu().numpy()
                flow_color_mujoco = flow_to_color(flow_2d_mujoco) # np array
                flow_visual = Image.fromarray((flow_color_mujoco * 255).astype(np.uint8))
                flow_visual.save(f'inspection_features1/ufm_flow_{i}.png')
                
                flow_color_mujoco = flow_to_color(flow_2d_mujoco - flow_2d_np_render) # np array
                flow_visual = Image.fromarray((flow_color_mujoco * 255).astype(np.uint8))
                flow_visual.save(f'inspection_features1/con_flow_{i}.png')
...
"""
python calibrate_bridge_single_ufm.py --model_path output/widow0 --scene_path /data/group_data/katefgroup/datasets/bridge_chenyu/raw/bridge_data_v1/berkeley/laundry_machine/put_clothes_in_laundry_machine/2022-03-19_14-53-07/raw/traj_group0/traj0
python calibrate_bridge_single_ufm.py --model_path output/widow0 --scene_path /data/group_data/katefgroup/datasets/bridge_chenyu/raw/bridge_data_v1/berkeley/toykitchen2_room8052/flip_orange_pot_upright_in_sink/2021-06-16_14-13-45/raw/traj_group0/traj0
python calibrate_bridge_single_ufm.py --model_path output/widow0 --scene_path /data/group_data/katefgroup/datasets/bridge_chenyu/raw/bridge_data_v2/datacol1_toykitchen1/many_skills/0/2023-03-15_14-35-28/raw/traj_group0/traj0
"""
...
if __name__ == "__main__":
    ...
    resume = False
    if n == 0:
        w2c = np.array([[ 0.5917, -0.8037, -0.0630, -0.1372],
                        [-0.6194, -0.4032, -0.6737,  0.1882],
                        [ 0.5161,  0.4376, -0.7363,  0.2387],
    ...
    if not resume:
        ...
        intrinsics = np.array(traj0_dict['intrinsic'])
        set_nested_dict(calib_dict, keys, {'w2c': w2c.tolist(), 'intrinsic': intrinsics.tolist()})
    ...
    model.eval()
    # images_ufm_source = torch.from_numpy(image_list_whole[:-1])
    # images_ufm_target = torch.from_numpy(image_list_whole[1:])
    ...
    for i_ufm, i_frame in enumerate(sampled_frames):
        ...
        source_image = torch.from_numpy(image_list_whole[i_frame])
        target_image = torch.from_numpy(image_list_whole[i_frame + 1])
        with torch.no_grad():
            ...
            # print(flow.shape, 'flow shape after ufm')
            # print(covisibility.shape, 'covisibility shape after ufm')
            ...
            flow_color_mujoco = flow_to_color(flow_2d_mujoco) # np array
            flow_visual = Image.fromarray((flow_color_mujoco * 255).astype(np.uint8))
            flow_visual.save(f'inspection_features1/ufm_flow_{i_ufm}.png')
    ...
    print(fwd_flows_whole.shape, 'fwd flows whole shape after ufm')
    print(fwd_valids_whole.shape, 'fwd valids whole shape after ufm')
    
    
    
    # c2w = np.linalg.inv(w2c)
    # Define perturbation ranges
    thetas_x = torch.linspace(-0.2, 0.2, 6)  # 3 steps, ±0.1 radians
    thetas_y = torch.linspace(-0.2, 0.2, 6)
    thetas_z = torch.linspace(-0.2, 0.2, 6)
    dxs = torch.linspace(-0.01, 0.01, 3)     # 3 steps, ±0.05 units
    dys = torch.linspace(-0.01, 0.01, 3)
    dzs = torch.linspace(-0.01, 0.01, 3)
    
    # thetas_x = torch.linspace(-0.2, -0.2, 1)  # 3 steps, ±0.1 radians
    # thetas_y = torch.linspace(-0.2, -0.2, 1)
    # thetas_z = torch.linspace(-0.1, -0.1, 1)
    # dxs = torch.linspace(-0.00, 0.00, 1)     # 3 steps, ±0.05 units
    # dys = torch.linspace(-0.00, 0.00, 1)
    # dzs = torch.linspace(-0.03, -0.03, 1)
    ...
    if not resume:
        for extrinsic in perturbed_extrinsics:
            ...
            search_idx = 2
            Optimize_list = [sampled_frames[search_idx], sampled_frames[search_idx]+1]
            ...
            fwd_flows = fwd_flows_whole[search_idx:search_idx+1]
            fwd_valids = fwd_valids_whole[search_idx:search_idx+1]
    ...
    for epoch in tqdm(range(epochs)):
        ...
        fingers = fingers_whole[Optimize_list]
        joints = joints_whole[Optimize_list]
        # fwd_flows = fwd_flows_whole[batch_idx:batch_idx + 1]
        # fwd_valids = fwd_valids_whole[batch_idx:batch_idx + 1]
        fwd_flows = fwd_flows_whole
        fwd_valids = fwd_valids_whole
        print(image_list.shape, 'images')
        print(fwd_flows.shape, 'fwd flows shape')
