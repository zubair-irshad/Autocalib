...
def compute_loss(optimize_camera, optimize_joints, camera_lr, joints_lr, optimization_steps, fwd_flows, fwd_valids):
    ...
    for step in tqdm(range(optimization_steps)):
        ...
        for camera, joint_pose in zip(all_cameras, all_joint_poses):
            ...
            new_image = output['render']
            mask_tensor = torch.sum((new_image - 1)**2, dim=0)
            sigma = 0.1  # Example value; adjust as needed
            mask_tensor = 1 - torch.exp(-mask_tensor / sigma**2)
            gaussian_masks.append(mask_tensor)
            i += 1
...
python calibrate_bridge_single_alltracker.py --model_path output/widow0 --scene_path /data/group_data/katefgroup/datasets/bridge_chenyu/raw/bridge_data_v2/datacol1_toykitchen1/many_skills/0/2023-03-15_14-35-28/raw/traj_group0/traj0
"""
